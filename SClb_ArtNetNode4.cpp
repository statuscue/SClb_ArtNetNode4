/*
 Name:		SClb_ArtNetNode4.cpp
 Created:	08-Mar-19 20:02:17
 Author:	statuscue
...
uint8_t SCArtNet4::_reply_buffer[ARTNET_REPLY_SIZE];

SCArtNet4::SCArtNet4(IPAddress address)
{
	initialize(0);
	setLocalIP(address);
...
SCArtNet4::SCArtNet4(IPAddress address, IPAddress subnet_mask)
{
	initialize(0);
	setLocalIP(address, subnet_mask);
}

SCArtNet4::SCArtNet4(IPAddress address, IPAddress subnet_mask, uint8_t* buffer)
{
	initialize(buffer);
	setLocalIP(address, subnet_mask);
...
void  SCArtNet4::initialize(uint8_t* b) {
	if (b == 0) {
		// create buffer
...
uint8_t  SCArtNet4::universe(void) {
	return _universe;
}

void SCArtNet4::setUniverse(uint8_t u) {
	_universe = u;
}

void SCArtNet4::setSubnetUniverse(uint8_t s, uint8_t u) {
	_universe = ((s & 0x0f) << 4) | (u & 0x0f);
}

void SCArtNet4::setUniverseAddress(uint8_t u) {
	if (u != 0x7f) {
		if (u & 0x80) {
...
void SCArtNet4::setSubnetAddress(uint8_t u) {
	if (u != 0x7f) {
		if (u & 0x80) {
...
void SCArtNet4::setNetAddress(uint8_t s) {
	if (s & 0x80) {
		_net = (s & 0x7F);
	}
}

void SCArtNet4::setLocalIP(IPAddress a) {
	_my_address = a;
	// Set IP Node
...
void SCArtNet4::setLocalIP(IPAddress a, IPAddress sn) {
	setLocalIP(a);
...
int  SCArtNet4::numberOfSlots(void) {
	return _dmx_slots;
}

void SCArtNet4::setNumberOfSlots(int n) {
	_dmx_slots = n;
}

uint8_t SCArtNet4::getSlot(int slot) {
	return _packet_buffer[ARTNET_ADDRESS_OFFSET + slot];
}

uint8_t SCArtNet4::getHTPSlot(int slot) {
	return _dmx_buffer_c[slot - 1];
}

void SCArtNet4::setSlot(int slot, uint8_t value) {
	_packet_buffer[ARTNET_ADDRESS_OFFSET + slot] = value;
}

uint8_t* SCArtNet4::dmxData(void) {
	return &_packet_buffer[ARTNET_ADDRESS_OFFSET + 1];
}

uint8_t* SCArtNet4::replyData(void) {
	return _reply_buffer;
}

char* SCArtNet4::shortName(void) {
	return (char*)&_reply_buffer[26];
}

char* SCArtNet4::longName(void) {
	return (char*)&_reply_buffer[44];
}

void SCArtNet4::setNodeName(const char* name) {
	//strcpy((char*)&_reply_buffer[26], name);
	strcpy(longName(), name);
}

uint8_t SCArtNet4::readDMXPacket(UDP* eUDP) {
...
uint8_t SCArtNet4::readDMXPacketContents(UDP* eUDP, int packetSize) {
	if (packetSize > 0) {
...
*/
uint16_t SCArtNet4::readArtNetPacket(UDP* eUDP) {
...
uint16_t SCArtNet4::readArtNetPacketContents(UDP* eUDP, int packetSize) {
	if (!_using_htp) {
		_dmx_slots = 0;
...
uint16_t SCArtNet4::readArtDMX(UDP* eUDP, uint16_t slots, int packetSize) {
	uint16_t opcode = ARTNET_NOP;
	if (_using_htp) {
...
void SCArtNet4::sendDMX(UDP* eUDP, IPAddress to_ip) {
...
*/
void SCArtNet4::send_art_poll_reply(UDP* eUDP) {
...
void SCArtNet4::send_art_tod(UDP* wUDP, uint8_t* todata, uint8_t ucount) {
...
void SCArtNet4::send_art_rdm(UDP* wUDP, uint8_t* rdmdata, IPAddress toa) {
...
void SCArtNet4::setArtTodRequestCallback(ArtNetDataRecvCallback callback) {
	_art_tod_req_callback = callback;
}

void SCArtNet4::setArtRDMCallback(ArtNetDataRecvCallback callback) {
	_art_rdm_callback = callback;
}

void SCArtNet4::setArtCommandCallback(ArtNetDataRecvCallback callback) {
	_art_cmd_callback = callback;
}

uint16_t SCArtNet4::parse_header(void) {
...
*/
uint16_t SCArtNet4::parse_art_address(UDP* wUDP) {
...
uint16_t SCArtNet4::parse_art_tod_request(UDP* wUDP) {
...
uint16_t SCArtNet4::parse_art_tod_control(UDP* wUDP) {
	if (_art_tod_req_callback != NULL) {
		if (_packet_buffer[21] == _net) {
...
uint16_t SCArtNet4::parse_art_rdm(UDP* wUDP) {
...
void SCArtNet4::parse_art_cmd(UDP* wUDP) {
...
void  SCArtNet4::initializePollReply(void) {
	int i;
	...
	strcpy((char*)&_reply_buffer[26], "SCArtNetDMX4");
	strcpy((char*)&_reply_buffer[44], "SCArtNetDMX4");
	...
	_reply_buffer[211] = 0x4;							//  BindIndex
